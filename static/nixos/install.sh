...
run_parted() {
  read -p "[?] We are going to to run parted on${bold} $INST_DEVICE ${normal}- is this okay? [Yes/No] " confirm < /dev/tty && [[ $confirm == [yY] || $confirm == [yY][eE][sS] ]] || exit 1
...
run_nixossetup(){
  ...
  wget -q https://dovh.cz/nixos/configuration-template.nix -O /mnt/etc/nixos/configuration.nix
...
print_finish(){
  echo
  echo
  echo Please take a note of following:
  echo
  echo
